...
export default async function Industries({ dict, locale }) {
  // WordPress verilerini çekiyoruz
  const wpData = await getHomePageData(locale);

  // Başlıklar
  const displayBadge = wpData?.industriesBadge || dict?.badge;
  const displayTitle = wpData?.industriesTitle || dict?.title;
  const displayBtnViewAll =
    wpData?.industriesBtn || dict?.btnViewAll || 'VIEW ALL';

  // 1. DİNAMİK LİSTE: WP'den gelen sektörlerden başlığı dolu olanları filtresiz alıyoruz
  const wpIndustries = (wpData?.industriesList || []).filter(
    (item) => item.title && item.title.trim() !== '',
  );

  // Varsayılan Arka Plan Görselleri (WP'de resim girilmezse veya fallback için)
  const defaultImages = [
  ...
  // 2. YEDEK LİSTE: WP boşsa senin orijinal 4 sektörün devreye girer
  const fallbackIndustries = [
    {
      title: dict?.ind1Title,
      description: dict?.ind1Desc,
      image: defaultImages[0],
    },
    {
      title: dict?.ind2Title,
      description: dict?.ind2Desc,
      image: defaultImages[1],
    },
    {
      title: dict?.ind3Title,
      description: dict?.ind3Desc,
      image: defaultImages[2],
    },
    {
      title: dict?.ind4Title,
      description: dict?.ind4Desc,
      image: defaultImages[3],
    },
  ];

  // Ekran verisini belirleme
  const finalIndustries =
    wpIndustries.length > 0
      ? wpIndustries.map((item, index) => ({
          title: item.title,
          description: item.desc,
          image: defaultImages[index % defaultImages.length], // İleride WP Image Field eklenirse buraya 'item.image' verilir
        }))
      : fallbackIndustries;

  // Izgara (Grid) sınıfını dinamik ayarlama
  const gridColsClass =
    finalIndustries.length === 2
      ? 'lg:grid-cols-2'
      : finalIndustries.length === 3
        ? 'lg:grid-cols-3'
        : 'lg:grid-cols-4';
  ...
  return (
    <section className="py-24 px-6 md:px-16 bg-customSurface">
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 ${gridColsClass} gap-6`}>
        {finalIndustries.map((industry, index) => (
          <div
            key={index}
